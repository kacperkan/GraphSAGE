...
def main():
    ...
    for node in graph.nodes():
        del graph.node[node]["feature"]
        del graph.node[node]["label"]

    saga = GraphSAGEAdapter()
    saga.fit(
        graph=graph,
        features=features,
        id_map=id_map,
        epochs=10,
        batch_size=32,
        max_total_steps=200,  # this is for testing only, leave default value
    )
    ...
    #     max_total_steps=200,
    # )
    # where `id_map` is created by mapping id -> id of nodes instead of
    # some own mapping

    # Note that embs[:, 128:] all equal zeros (the matrix itself is Nx256)
